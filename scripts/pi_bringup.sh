...
WITH_TEDDY_APPROACH="${WITH_TEDDY_APPROACH:-0}"
WITH_IMU="${WITH_IMU:-1}"
WITH_MEGA_DRIVER="${WITH_MEGA_DRIVER:-1}"
WITH_EKF="${WITH_EKF:-1}"
WITH_LIDAR_SCAN_MATCHER="${WITH_LIDAR_SCAN_MATCHER:-0}"
PC_HOST="${PC_HOST:-}"
...
MIN_NONZERO_PWM="${MIN_NONZERO_PWM:-55}"
PURE_ROTATION_LINEAR_DEADBAND_MPS="${PURE_ROTATION_LINEAR_DEADBAND_MPS:-0.03}"
...
docker "${docker_run_args[@]}" \
  ...
  ros bash -lc "source /opt/ros/jazzy/setup.bash && source /ws/install/setup.bash && ros2 launch robot_bringup pi_robot.launch.py use_nav2:=${WITH_NAV2} use_teddy:=${WITH_TEDDY} use_teddy_approach:=${WITH_TEDDY_APPROACH} use_imu:=${WITH_IMU} use_mega_driver:=${WITH_MEGA_DRIVER} use_ekf:=${WITH_EKF} use_lidar_scan_matcher:=${WITH_LIDAR_SCAN_MATCHER} product_name:=${PRODUCT_NAME} port_name:=${PORT_NAME} port_baudrate:=${PORT_BAUDRATE} frame_id:=${LIDAR_FRAME} base_frame:=${BASE_FRAME} imu_frame:=${IMU_FRAME} mega_port:=${MEGA_PORT} mega_baudrate:=${MEGA_BAUDRATE} mega_odom_topic:=${MEGA_ODOM_TOPIC} mega_publish_tf:=${MEGA_PUBLISH_TF} swap_sides:=${SWAP_SIDES} left_cmd_sign:=${LEFT_CMD_SIGN} right_cmd_sign:=${RIGHT_CMD_SIGN} angular_cmd_sign:=${ANGULAR_CMD_SIGN} min_nonzero_pwm:=${MIN_NONZERO_PWM} pure_rotation_linear_deadband_mps:=${PURE_ROTATION_LINEAR_DEADBAND_MPS} left_cmd_scale:=${LEFT_CMD_SCALE} right_cmd_scale:=${RIGHT_CMD_SCALE} left_tick_sign:=${LEFT_TICK_SIGN} right_tick_sign:=${RIGHT_TICK_SIGN} left_m_per_tick:=${LEFT_M_PER_TICK} right_m_per_tick:=${RIGHT_M_PER_TICK} track_width_eff_m:=${TRACK_WIDTH_EFF_M} ekf_params_file:=${EKF_PARAMS_FILE} params_file:=${PARAMS_FILE} teddy_approach_params_file:=${TEDDY_APPROACH_PARAMS_FILE}"
