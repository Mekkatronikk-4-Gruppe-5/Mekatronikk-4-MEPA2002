...
class SimKeyboardTeleop:
    def __init__(self, args: argparse.Namespace) -> None:
        self.node = rclpy.create_node("sim_keyboard_teleop")
        ...
        self.root = tk.Tk()
        self.root.title("Sim Keyboard Teleop")
        self.root.geometry("560x260")
        self.root.configure(bg="#111111")
        self.root.protocol("WM_DELETE_WINDOW", self.close)

        self.status_var = tk.StringVar(value="Connected to /cmd_vel")
    ...
    def _tick(self) -> None:
        ...
        should_send = command != self.last_command or (should_repeat and now - self.last_sent_at >= self.send_period)
